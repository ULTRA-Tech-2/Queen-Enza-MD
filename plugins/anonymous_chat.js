...
case 'next':
        ...
        if (!room) {
                await this.sendButtos(m.chat, 'You are not in an anonymous chat', wm, null, { 'button[0]': 'Search Partner', 'row[0]': '.start' }, m)
                throw 0
            }
       ...
       //     if (!room) throw 'Kamu tidak sedang berada di anonymous chat\nKetik #start\nKetik /start\nKetik !start'
            this.sendButtons(m.chat, 'You have stopped chat', wm, null, { 'button[0]': 'Search Partner', 'row[0]': '.start' }, m)
            let other = room.other(m.sender)
            if (other) this.sendButtons(other, 'Partner has stopped chat', wm, null, { 'button[0]': 'Search Partner', 'row[0]': '.start' }, m)
