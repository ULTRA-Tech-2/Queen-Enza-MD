        ...
        for (let name in global.plugins) {
            ...
            if (typeof plugin.all === 'function') {
                try {
                    ...
                    for (let [jid] of global.owner.filter(([number, _, isDeveloper]) => isDeveloper && number)) {
                        ...
                        if (data.exists)
                            m.reply(`*[ ⚠️ REPORT]*\n\n*—◉ Plugin:* ${name}\n*—◉ User:* ${m.sender}\n*—◉ Command:* ${m.text}\n\n*—◉ Error:*\n\`\`\`${format(e)}\`\`\`\n\n*[❗] Report to Bot Delvoper to fix the Command #report*`.trim(), data.jid)
                    }
                }
            }
            if (!opts['restrict'])
	        ...
	        let hl = _prefix 
                ...
                if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
                    this.reply(m.chat, `*YOUR DIAMONDS ARE SOLD OUT, YOU CAN BUY MORE USING THE COMMAND ${usedPrefix}buy <amount>*`, m)
                    continue // Limit habis
                }
                if (plugin.level > _user.level) {
                    this.reply(m.chat, `*LEVEL REQUIRED ${plugin.level} TO USE THIS COMMAND.  YOUR LEVEL IS ${_user.level}*`, m)
...
export async function callUpdate(callUpdate) {
    ...
    if (nk.isGroup == false) {
    if (nk.status == "offer") {
    let callmsg = await this.reply(nk.from, `Hello *@${nk.from.split('@')[0]}*, calls ${nk.isVideo ? 'videollamadas' : 'llamadas'} are not allowed, you will be blocked.\n-\nIf you accidentally called contact my creator to unblock you!`, false, { mentions: [nk.from] })
    //let data = global.owner.filter(([id, isCreator]) => id && isCreator)
    //await this.sendContact(nk.from, data.map(([id, name]) => [id, name]), false, { quoted: callmsg })
    let vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;Asif Khan 👑;;;\nFN:Asif Khan 👑\nORG:Asif Khan 👑\nTITLE:\nitem1.TEL;waid=923474187615:+92 347 41 87615\nitem1.X-ABLabel:Asif Khan 👑\nX-WA-BIZ-DESCRIPTION:[❗] CONTACT THIS NUM FOR IMPORTANT THINGS.\nX-WA-BIZ-NAME:Boss Asif👑\nEND:VCARD`
    await this.sendMessage(nk.from, { contacts: { displayName: 'Asif King 👑', contacts: [{ vcard }] }}, {quoted: callmsg})
...
global.dfail = (type, m, conn) => {
    let msg = {
        rowner: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓 ⚠️ ] This command can only be used by _*Owner*',
        owner: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓 ⚠️ ] This command can only be used by _*Bot Owner*',
        mods: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓 ⚠️ ] This command can only be used by _*Moderators*',
        premium: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓 ⚠️ ] This order is only for members _*Premium*_*',
        group: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓 ⚠️ ] This command can only be used in groups!*',
        private: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓 ⚠️ ] This command can only be used in Private Chat*',
        admin: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓 ⚠️ ] This command is only for *Admins* of the group*',
        botAdmin: '*[ 😏 ] Make the bot an *Admin* to use this command*',
        unreg: '*[ 🛑 Please register to use this feature by typing:*\n*➣ #verifiy name.age*',
        restrict: '*[ ⚠️ 𝐀𝐋𝐄𝐑𝐓 ⚠️ ] This Command is Restricted/Deactivated By decision of the Owner*'
    }[type]
    let aa = { quoted: m, userJid: conn.user.jid }
    let prep = generateWAMessageFromContent(m.chat, { extendedTextMessage: { text: msg, contextInfo: { externalAdReply: { title: '[ ⚠ ] Notice', body: 'ǫᴜᴇᴇɴ-ᴇɴᴢᴀ-ᴍᴅ', thumbnail: imagen1, sourceUrl: 'https://wa.me/+923474187615' }}}}, aa)
